//Perlin noise code stolen from https://gist.github.com/nowl/828013
...
{
	int tmp = hash[(y + SEED) & 0xFF];
	return hash[(tmp + x + (SEED>>8)) & 0xFF];
}
...
void make_single_path() {
	...
	while (i<PATH_LEN && x>0 && x<WID && y>0 && y<HEI) {
		...
		float angle = 2 * M_PI * (perlin2d(x, y, frequency, 6) - 0.5) * curvature;
		x += cos(angle);
		y += sin(angle);
		i++;
	}
...
void draw() {
	curvature = 0.4 + 0.6 * ((float) (rand() & 0xFF)) / 0xFF;
	frequency = 0.005 + 0.005 * ((float) (rand() & 0xFF)) / 0xFF;
	int i;
